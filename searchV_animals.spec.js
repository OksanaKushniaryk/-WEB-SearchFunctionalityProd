...
describe('searchV', function() {
  ...
  it('searchV', async function() {
    ...
    await driver.findElement(By.xpath("(//input[@type='text'])[2]")).sendKeys("animals")
    ...
    // Count items with "animals" in their title
    let animalsCount = 0
    for (const title of titles) {
        const text = await title.getText()
        if (text.toLowerCase().includes('animals')) {
            animalsCount++
        }
    }
    
    // Log UI results
    console.log(`\nUI Results: Found ${animalsCount} items with "animals" in title`)
    
    // Assert both API and UI conditions
    assert.ok(isSorted, 'API results should be sorted by updated date in descending order')
    assert.ok(animalsCount > 0, 'Should find at least one item with "animals" in title')
